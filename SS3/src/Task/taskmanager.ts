...
export default class TaskManager {
  ...
  addTask(title: string, deadline: string): void {
    ...
    console.log(
      `Id: ${this._tasks.length}, Title: ${title}, Deadline: ${deadline}`
    );
  }
  ...
  assignTask(employeeId: number, taskId: number): void {
    ...
    this._assignments.push(
      new Assignment(this._employees[employeeId - 1], this._tasks[taskId - 1])
    );
    alert("Phân công công việc thành công");
    console.log(`Phân công công việc cho nhân viên như sau:
        Id nhân viên: ${employeeId} - Name: ${
      this._employees[employeeId - 1].name
    }
        Id Task: ${taskId} - Title: ${
      this._tasks[taskId - 1].title
    } - Dealine: ${this._tasks[taskId - 1].deadline} - IsCompleted: ${
      this._tasks[taskId - 1].isCompleted
    }`);
  }
  ...
  listTasks(): void {
    ...
    this._tasks.forEach((task) => {
      ...
      let now = new Date();
      let isOverdue =
        deadline.getTime() < now.getTime() ? "Quá hạn" : "Chưa quá hạn";
      alert(
        `ID: ${task.id}, Tiêu đề: ${task.title}, Deadline: ${task.deadline}, Trạng thái: ${status}, ${isOverdue}`
      );
      console.log(
        `ID: ${task.id}, Tiêu đề: ${task.title}, Deadline: ${task.deadline}, Trạng thái: ${status}, ${isOverdue}`
      );
